...
function drawFooter(doc, fonts, invoice, y) {
    const leftX = 42;
    const rightX = 355;
    const orderDisplayNumber = formatOrderDisplayNumber(invoice);
    ...
    text(doc, `Order No: #${orderDisplayNumber}`, leftX, 812, { font: fonts.regular, size: 7, color: "#666666", width: 500 });
}
...
function renderInvoicePdf(invoice, outputPath = createLocalInvoicePath(invoice.invoiceNumber)) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const invoiceDateY = invoiceNumberY + invoiceNumberHeight + 14;
