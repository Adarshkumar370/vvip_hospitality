...
function getInvoiceBucket() {
    return process.env.STORAGE_S3_BUCKET || "vvip-bucket";
}
...
function getPublicInvoiceUrl(bucket, key) {
    const publicUrl = process.env.STORAGE_S3_PUBLIC_URL || process.env.STORAGE_S3_ENDPOINT;
    if (!publicUrl) {
        throw new Error("STORAGE_S3_PUBLIC_URL or STORAGE_S3_ENDPOINT is required to build the public invoice URL");
    }
    ...
    return db.begin(async (tx) => {
        ...
        if (Number(itemCount.count) === 0) {
            await tx`
                ...
                SELECT
                    ${invoice.id}, oi.product_id, oi.id, oi.line_number,
                    oi.product_name_snapshot, oi.category_name_snapshot, oi.unit_code_snapshot, oi.unit_symbol_snapshot,
                    oi.quantity, oi.unit_price, oi.discount_amount, oi.tax_amount,
                    p.hsn_code, COALESCE(p.gst_rate, 0)
                FROM order_items oi
...
async function getInvoicePdfPayload(db, invoiceId) {
    ...
    const addressSnapshot = invoice.billing_address_snapshot || invoice.delivery_address_snapshot;
    ...
    return {
        ...
        billingAddress: compactAddress(addressSnapshot),
        billingMobile: invoice.billing_mobile_no || addressSnapshot?.recipient_mobile_no || "",
        billingEmail: invoice.billing_email || "",
        placeOfSupply: addressSnapshot?.state || "",
